...
export default function MisApuestasClient() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from("bets")
      .select("id,sport,event_label,pick_label,color,prob_snapshot,odds_taken,stake,stake_source,status,profit,placed_at")
  ...
  async function addBet() {
    ...
    if (!Number.isFinite(stakeNum) || stakeNum <= 0) return alert("Stake inválido");
    if (!Number.isFinite(oddsNum) || oddsNum <= 1) return alert("Cuota inválida (debe ser > 1)");
    if (!Number.isFinite(probNum) || probNum <= 0 || probNum >= 1) return alert("Prob inválida (0-1)");
    ...
    if (lErr) return alert("Apuesta creada pero error en ledger: " + lErr.message);
    ...
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ background: "white", border: "1px solid #eee", borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ display: "grid", gap: 10, marginTop: 12, maxWidth: 640 }}>
          ...
          <button
            onClick={addBet}
            style={{ padding: 10, borderRadius: 10, background: "#111", color: "white", fontWeight: 800, cursor: "pointer" }}
          >
            Añadir apuesta
          </button>
        </div>
      ...
      <div style={{ background: "white", border: "1px solid #eee", borderRadius: 12, padding: 16 }}>
        ...
        {loading ? (
          ...
          <div style={{ marginTop: 10, color: "#666" }}>Aún no tienes apuestas.</div>
        ) : (
              ...
              <tbody>
                {bets.map((b) => (
                  <tr key={b.id} style={{ borderBottom: "1px solid #f2f2f2" }}>
                    ...
                    <td style={{ padding: 8 }}>{Number(b.odds_taken).toFixed(2)}</td>
                    <td style={{ padding: 8 }}>{Number(b.stake).toFixed(2)}€</td>
                    <td style={{ padding: 8 }}>{b.status}</td>
                    <td style={{ padding: 8 }}>{Number(b.profit).toFixed(2)}€</td>
                  </tr>
                ))}
              </tbody>
            </table>
